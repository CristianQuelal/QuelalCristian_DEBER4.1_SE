...
void loop() {
  if(digitalRead(btn)==LOW)                 //control mediante el botón
  {
      ...
      lcd.setCursor(5,0);                   //ubicacion del cursor
      lcd.print(conversor);                 //impresion del mensaje en la LCD
      lcd.setCursor(0,1);                   //ubicacion del cursor
      lcd.print("V: ");                     //impresion del mensaje en la LCD
      lcd.setCursor(3,1);                   //ubicacion del cursor
      lcd.print(voltaje);                   //impresion del mensaje en la LCD
      lcd.setCursor(9,1);                   //ubicacion del cursor
      lcd.print("PWM: ");                   //impresion del mensaje en la LCD
      lcd.setCursor(13,1);                  //ubicacion del cursor
      lcd.print((dutty*100)/255);           //impresion del mensaje en la LCD
      delay(300);
      lcd1.clear();
  }else{
    lcd1.clear();
  ...
  if(digitalRead(SW)==HIGH)                   //Control mediante Switch
  {
      ...
      lcd1.print(conversor1);                 //impresion del mensaje en la LCD
      lcd1.setCursor(0,1);                    //ubicacion del cursor en la lcd
      lcd1.print("V: ");                      //impresion del mensaje en la LCD
      lcd1.setCursor(3,1);                    //ubicacion del cursor en la lcd
      lcd1.print(voltaje1);                   //Se imprime en dicha posición.
      lcd1.setCursor(9,1);                    //ubicacion del cursor en la lcd
      lcd1.print("PWM: ");                    //impresion del mensaje en la LCD
      lcd1.setCursor(13,1);                   
